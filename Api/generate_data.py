...
#Alteração do setup_base_data para não dar erro quando já houver um id
...
def setup_base_data(conn):
    ...
    # 1️ Garante que a marca base exista ou cria se não existir
    cursor.execute("SELECT id FROM brands WHERE id = %s", (BRAND_ID,))
    if cursor.fetchone() is None:
        cursor.execute(
            "INSERT INTO brands (id, name) VALUES (%s, %s)",
            (BRAND_ID, 'Nola God Level Brand')
        )
    else:
        print(f"✓ Brand with id={BRAND_ID} already exists, skipping creation.")

    # 2️ Sub-brands (verifica se já existem)
    sub_brands = ['Challenge Burger', 'Challenge Pizza', 'Challenge Sushi']
    sub_brand_ids = []

    for sb in sub_brands:
        cursor.execute(
            "SELECT id FROM sub_brands WHERE brand_id = %s AND name = %s",
            (BRAND_ID, sb)
        )
        existing = cursor.fetchone()
        if existing:
            sub_brand_ids.append(existing[0])
        else:
            cursor.execute(
    ...
    # 3️ Channels (evita duplicados)
    channel_ids = []
    for name, ch_type, weight, commission in CHANNELS:
        cursor.execute(
            "SELECT id FROM channels WHERE brand_id = %s AND name = %s",
            (BRAND_ID, name)
        )
        existing = cursor.fetchone()
        if existing:
    ...
    # 4️ Payment types (também verifica duplicados)
    for pt in PAYMENT_TYPES_LIST:
        cursor.execute(
            "SELECT id FROM payment_types WHERE brand_id = %s AND description = %s",
            (BRAND_ID, pt)
        )
        if cursor.fetchone() is None:
            cursor.execute(
